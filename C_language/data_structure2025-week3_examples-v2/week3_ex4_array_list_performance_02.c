...
/**** delete last 수행시간 측정 ****/
    ...
    for (int i = 0; i < REPEAT_CNT; i++) {
        if (array_list_delete_last(mylist) == false) {
...
/**** delete first 수행시간 측정 ****/
    ...
    start = clock();    // 시간 측정 시작
    for (int i = 0; i < REPEAT_CNT; i++) {
        if (array_list_delete_first(mylist) == false) {
            printf("Failed to delete first at %d!\n", i);
            return 1;
        }
    }
    ...
    // 수행시간 출력
    printf("(delete first) Elapsed time: %lf seconds\n", (double)(end - start) / CLOCKS_PER_SEC);
    // arraylist 소멸 (잊지 말자 free!에 해당)
    if (array_list_destroy(&mylist) == false) {
        printf("Failed to destroy array list!\n");
        return 1;
    }
/**** delete first 수행시간 측정 끝 ****/
...
/**** delete index (현재 길이의 중앙 위치에) 수행시간 측정 ****/
    ...
    start = clock();    // 시간 측정 시작
    for (int i = 0; i < REPEAT_CNT; i++) {
        if (array_list_delete_index(mylist, array_list_get_size(mylist)/2) == false) {
    ...
    // 수행시간 출력
    printf("(delete index) Elapsed time: %lf seconds\n", (double)(end - start) / CLOCKS_PER_SEC);
    // arraylist 소멸 (잊지 말자 free!에 해당)
    if (array_list_destroy(&mylist) == false) {
        printf("Failed to destroy array list!\n");
        return 1;
    }
/**** delete index 수행시간 측정 끝 ****/
