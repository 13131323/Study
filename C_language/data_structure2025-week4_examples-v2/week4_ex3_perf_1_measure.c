/*
 * Performance measurement: Selection Sort
 */
...
void print_elapsed_time(int n, double elapsed);

void selection_sort(element_t *data, int n) {
    int i;       // 몇 번째 최소값을 찾을 것인지
    int j;       // 최소값을 탐색을 위해 움직일 변수
    int min_idx; // 1번의 탐색에서 현재까지의 최소값의 인덱스를 저장할 변수
    element_t temp;    // swap을 위한 변수
...
/**** 2배씩 늘려가며 시간 측정 ****/
    // selection sort 수행 (selection sort에서 그 뒤는 너무 오래걸리니까 2^17까지)
    for (i = 250; i <= cnt && i <= (1<<17); i *= 2) {
        start = clock();
        selection_sort(myarr, i);
        end = clock();
        print_elapsed_time(i, (double)(end - start) / CLOCKS_PER_SEC);
        // printf("Elapsed time for %d elements: %lf seconds\n", i, (double)(end - start) / CLOCKS_PER_SEC);
    }
/********************************/
...
// 수행시간 출력을 위한 함수(자료구조 수업과는 큰 관련은 없습니다.)
void print_elapsed_time_1(int n, double elapsed) {
    static int cnt = 0;
    if (cnt++ == 0) {
...
void print_elapsed_time_2(int n, double elapsed) {
    static int cnt = 0;
    if (cnt++ == 0) {
        printf("****** N vs Running Time (in second) ******\n");
        printf("┌────────┬──────────┐\n");
...
void print_elapsed_time(int n, double elapsed) {
    print_elapsed_time_1(n, elapsed);
    // print_elapsed_time_2(n, elapsed);
}