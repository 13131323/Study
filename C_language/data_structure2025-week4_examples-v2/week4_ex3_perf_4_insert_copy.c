/*
 * Performance measurement: Insertion Sort 원본의 데이터를 유지해주어야 제대로된 측정
 */
...
/**** 2배씩 늘려가며 시간 측정 ****/
    ...
    for (i = 250; i <= cnt && i <= (1<<17); i *= 2) {
        ...
        double elapsed = 0.0;
        while (1) {
            element_t *copy = (element_t *)malloc(sizeof(element_t) * i);
            if (copy == NULL) {
                printf("Failed to allocate memory!\n");
                free(myarr);
                return 1;
            }
            memcpy(copy, myarr, sizeof(element_t) * i);
        ...
        // 조금더 정확히 측정하기 위해서는 insertion sort외의 수행시간을 구한 뒤 제외하는게 좋음
        // elapsed = 0.0;
        // int repeat2 = 0;
        // start = clock();
        // while (1) {
        ...
        // double avg2 = elapsed / repeat2;
        // print_elapsed_time(i, avg - avg2, repeat);
    }
/********************************/
